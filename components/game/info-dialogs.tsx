...
export function StatisticsDialog({ stats, today }: { stats: GameStats; today: string }) {
  ...
  const values = [
    ["Played", stats.startedDates.length], ["Completed", completed], ["Current streak", streak.current],
    ["Max streak", streak.maximum], ["Average guesses", average], ["Best", stats.bestGuessCount ?? "—"],
  ];
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <dl className="grid grid-cols-3 gap-2">{values.map(([label, value]) => <div key={label} className="rounded-lg border bg-muted/35 p-3 text-center"><dd className="font-mono text-xl font-semibold text-primary">{value}</dd><dt className="mt-1 text-[11px] text-muted-foreground">{label}</dt></div>)}</dl>
